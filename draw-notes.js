...
const midiContainerWidth = 900; // Fixed width
const midiContainerHeight = 400; // Fixed height
...
function adjustOnResize() {
    ...
    const newWidth = midiContainer.offsetWidth;
    const newHeight = midiContainer.offsetHeight;
    
    // Update global variables for the new container size
    rectWidth = (newWidth / 8) - gap_X;
    rectHeight = (newHeight - (HIGHEST_NOTE - LOWEST_NOTE) * (gap_Y)) / (HIGHEST_NOTE - LOWEST_NOTE + 1);
    
    // Redraw rectangles with new dimensions
    // This may involve clearing the existing rectangles and reinitializing them
}
...
for (let i = 0; i < chordOutputKeyList.length; i++) {
    ...
    for (let j = 0; j < chordOutput[key]["notes"].length; j++) {
        const currentNoteRect = new NoteRectangle(key, chordOutput[key], randomIntInRange(40,74), 'audio'); //chordOutput[key]["notes"][j]
